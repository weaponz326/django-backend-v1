...
# # checks if user has a hotel acount
...
class ActiveAccountView(APIView):
    def post(self, request, *args, **kwargs):
        request.session['active_hotel'] = True
        request.session['hotel_id'] = request.data.get('active_account')

        return Response({
            'active_hotel': request.session['active_hotel'],
...
# # get all hotel suites of a personal id
