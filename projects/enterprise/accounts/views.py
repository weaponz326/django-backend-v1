...
# checks if user has a enterprise acount
...
class ActiveAccountView(APIView):
    def post(self, request, *args, **kwargs):
        request.session['active_enterprise'] = True
        request.session['enterprise_id'] = request.data.get('active_account')

        return Response({
            'active_enterprise': request.session['active_enterprise'],
...
# get all enterprise suites of a personal id
