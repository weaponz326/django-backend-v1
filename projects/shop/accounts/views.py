...
# # checks if user has a shop acount
...
class ActiveAccountView(APIView):
    def post(self, request, *args, **kwargs):
        request.session['active_shop'] = True
        request.session['shop_id'] = request.data.get('active_account')

        return Response({
            'active_shop': request.session['active_shop'],
...
# # get all shop suites of a personal id
