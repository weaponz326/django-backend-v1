...
class ActiveAccountView(APIView):
    def post(self, request, *args, **kwargs):
        request.session['active_production'] = True
        request.session['production_id'] = request.data.get('active_account')

        return Response({
            'active_production': request.session['active_production'],
